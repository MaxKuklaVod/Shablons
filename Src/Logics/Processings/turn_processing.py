...
class turn_processing(processing):
    ...
    def process(self, source: list) -> list:
        ...
        for key, transactions in grouped_transactions.items():
            ...
            # Расчитываем оборот
            turnover = sum(
                transaction.value if transaction.storage_type else -transaction.value
                for transaction in transactions
            )

            # Создаем модель
            row = storage_row_turn_model.create(
                first_transaction.nomenclature,
                first_transaction.storage,
                first_transaction.unit,
            )
            row.value = turnover
