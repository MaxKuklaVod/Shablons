...
from Src.Logics.Services.console_log import console_log
...
class start_factory:
    ...
    def create_receipts(_data: list = None) -> list:
        ...
        item = receipe_model.create_receipt(
            "Вафли хрустящие в вафильнице", "", items, data
        )
    ...
    def create_storage_transactions(data: dict) -> list:
        ...
        if len(data.keys()) == 0:
            raise operation_exception(
                "Набор данных пуст. Невозможно сформировать список транзакций!"
            )

        items = [
            ("Мука пшеничная", 1, "киллограмм"),
        ...
        for element in items:
            if len(element) < 3:
                raise operation_exception(
                    "Некорректно сформирован список для генерации рецептов!"
                )
            ...
            row = storage_row_model.create_credit_row(
                nomenclature_name, quantity, unit_name, data, default_storage
            )
            result.append(row)
    ...
    def create(self) -> bool:
        ...
        if self.__oprions.is_first_start == True:
            ...
            try:
                ...
                raise operation_exception(
                    f"Ошибка при формировании шаблонных данных!\n{ex}"
                )
