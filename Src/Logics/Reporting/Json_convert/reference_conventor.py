...
from Src.Logics.Reporting.Json_convert.abstract_convertor import abstract_convertor
from Src.Logics.Reporting.Json_convert.basic_conventor import basic_conventor
from Src.Logics.Reporting.Json_convert.date_time_convertor import date_time_conventor
...
class reference_conventor(abstract_convertor):
    ...
    def __build_structure(self):
        ...
        self.__converts = {
            datetime: self.__convert_dt,
            dict: self.__convert_dict,
            type(None): self.__convert_None,
        }
    ...
    def convert(self, reference):
        ...
        fields = list(
            filter(
                lambda x: not x.startswith("_") and not x.startswith("create_"),
                dir(reference.__class__),
            )
        )
