...
class convert_factory:
            ...
            if isinstance(attribute, property):
                ...
                try:
                    ...
                    raise operation_exception(
                        f"Невозможно сериализовать объект в набор словарей. Поле {field}, значение: {dictionary}"
                    )
    ...
    def __serialize_item(self, field: str, source):
        ...
        if type(source) not in self._maps.keys():
            raise operation_exception(
                f"Не возможно подобрать конвертор для типа {type(source)}"
            )
        ...
        if not convertor.is_empty:
            raise operation_exception(
                f"Ошибка при конвертации данных {convertor.error}"
            )
