...
class test_convert(unittest.TestCase):
    ...
    def test_convert_nomenclature(self):
        ...
        item = reference_conventor(
            type(factory.storage.data[storage.nomenclature_key()][0]),
            type(error_proxy()),
            type(factory.storage.data[storage.group_key()][0]),
            type(factory.storage.data[storage.unit_key()][0]),
        )
    ...
    def test_convert_reciepe(self):
        ...
        print(
            type(
                list(
                    (
                        factory.storage.data[storage.reciepe_key()][
                            0
                        ].ingridient_proportions
                    ).values()
                )[0][2]
            )
        )
